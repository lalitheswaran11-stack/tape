...
| `POST /fault/stall` | `{ ms: 3000 }` | Upstream freeze: sending **and generating** stop for the window — including pong replies, so the server is fully silent; seq stays contiguous and the pacing clock resets on resume, so there is no catch-up burst. | Ride through stalls shorter than the heartbeat window. A longer stall is indistinguishable from a silently dead socket: declare `degraded`, force-reconnect, resync. |
